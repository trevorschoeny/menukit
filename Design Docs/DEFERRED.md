...
- ~~**Click-through prohibition for ALL panels (architectural — surface to advisor).**~~ **RESOLVED in Phase 14d-2.5 (M9).** Trevor's principle (`"everything behind the panel is completely inert"`) shipped as the M9 mechanism: `Panel.opaque(boolean)` flag with default `true`, library-wide universalization of the four 14d-1 modal mixins to consult bounds-based "any visible opaque panel covers this point" instead of "any visible modal." Modal reframed as composition over three independent flags (`opaque + dimsBehind + tracksAsModal`). See `Design Docs/Mechanisms/M9_PANEL_OPACITY.md` for full design + verdicts.

- ~~**Lambda-adapter modal-aware extension.**~~ **RESOLVED in Phase 14d-2.5 (M9).** Lambda-path adapters get `.activeOn(Screen, Supplier<ScreenBounds>)` + `.deactivate(Screen)` lifecycle to register with the unified opacity registry. Library extends opacity coverage to lambda panels automatically; escape-hatch property (custom positioning, consumer-owned dispatch) preserved. Consumers using the lambda path call `.activeOn(...)` from their mixin's `init()` to participate. See M9 §4.4 for the design.

## Phase 14d-2.5 follow-on items

Surfaced during the M9 design draft (Phase 14d-2.5). Full design lives in `Design Docs/Mechanisms/M9_PANEL_OPACITY.md`.

- **Per-element opacity (primitive-gap candidate).** M9 makes opacity a per-Panel property — every element within an opaque panel inherits the opacity. A consumer wanting a "click-through hole" within an otherwise-opaque panel — e.g., a highlight reticle that opaque-covers most of an inventory but lets clicks reach specific slots underneath — has no v1 capability. **Trigger:** first concrete consumer naming the use case. Architectural shape questions to resolve at trigger time: (a) per-element opacity flag (each element declares its own opacity, default opaque, transparent elements pass clicks through to whatever's behind THEM); (b) `Panel.opaque(true)` with element-exception list (panel-level opaque; named elements pass through); (c) composition shape — consumer-coordinated transparent regions via Panel positioning rather than per-element opacity; (d) some shape from the consumer's actual need that none of (a)/(b)/(c) anticipates. Right discipline: surface gap with trigger condition rather than ignoring or pre-designing.
...
- ~~**Server-menu Hub entries (primitive-gap candidate — C2S packet plumbing).**~~ **RESOLVED in 14d-2.7.** The `OpenScenarioC2SPayload` shape covers server-menu Hub entries cleanly. ElementDemo specifically was deleted (V1.1 Palette Matrix + V1.2 Composed cover the same evidence territory).

- **Open-registry Hub** (deferred-on-evidence). The validator's Hub uses a hand-coded entry list (`HubHandler.create` with newest-first source order). If a future consumer wants to extend the Hub from outside validator (e.g., a sister test mod), an open-registry pattern (`TestHub.add(label, onClick)` + registration-order reversal on display) would be the architectural shape. Not needed today; the hand-coded list is simpler. **Trigger:** first concrete out-of-validator consumer wanting to add Hub entries.
