...
- **Keybind-category-sharing review (Phase 15 public-release prep).** §11 scope-down preserved `getKeybindCategory()` as Layer A on the rationale that grouping is a real user-facing coordination primitive. Question that surfaced during M3 smoke: when a non-Trevor author adopts MenuKit, their mod isn't necessarily part of a "family" — MenuKit shipping a grouping primitive may be the same shape of ecosystem-shaping assumption Layer B was. Trigger: public-release prep begins. If the answer is "keybind-sharing leaves too," MKFamily's Layer A shrinks further to identity + mod-id roster only.

## Phase 14d-1 follow-on items

Surfaced during the 14d-1 dialog primitive design + Commit 1 implementation prep.
Full design rationale lives in `Design Docs/Elements/DIALOGS.md` §4.5 + §4.9 + §10.

- **Multi-line / wrapped TextLabel variant.** Dialog body text is single-line in v1 — the dialog builder accepts a single `Component` for body, and consumers needing multi-line wording compose `Column.of(TextLabel.spec(line1)).add(TextLabel.spec(line2))...build()` manually using the same M8 patterns the builder uses internally. The escape hatch is documented in DIALOGS.md §5. Real new primitive that warrants its own evidence-driven phase. **Trigger:** 3+ concrete consumer cases needing wrapped text (dialog bodies are case 1; V7-style HUD hint multi-line is case 2-candidate). Design surface for the variant: wrapping algorithm (word-break vs hard-break), baseline alignment, auto-sizing semantics, max-width declaration, line-height parameter. Not Phase 14d-1 scope.

- **MenuKit-native screen dispatch for dialogs.** 14d-1 ships dialog support for the MenuContext path (vanilla container screens via `ScreenPanelAdapter`). MenuKit-native screens (`MenuKitScreen` subclasses) need a different architectural call: `ScreenPanelRegistry.java:212` early-returns for non-`AbstractContainerScreen` instances, and `ScreenPanelAdapter.render(...)` requires `AbstractContainerScreen` as a parameter. Two architectural shapes worth verdict at trigger time: (α) generalize ScreenPanelAdapter to dispatch on any Screen subclass; (β) native dispatch on MenuKitScreen via direct panel-list integration plus a way to bypass `PanelLayout` constraint flow for modal panels. **Trigger:** first concrete consumer in Phase 15 (sandboxes selector wanting a modal dialog, or IP equipment screen wanting one). Round-2 advisor verdict: defer (γ) — Phase 14d-1 already expanded scope to ship full modality (Finding C); adding MenuKit-native dispatch on top is too much for one phase.

- ~~**Dim-behind overlay for modal dialogs.**~~ **SHIPPED in 14d-1 round 2.** Trevor's smoke verdict: dialogs without dim "not visually distinct enough." Translucent black quad (`0xC0000000` ~75% black) rendered before modal panel background, full-screen-window. Implemented in `ScreenPanelAdapter.render` gated on `panel.cancelsUnhandledClicks(true)`. The §4.10 deferred-pending-smoke discipline did its job — smoke said yes, fold-inline shipped.

- **Keyboard suppression for modal dialogs.** Round-2 verdict: ship modal v1 without keyboard suppression. Consumer's `onConfirm`/`onCancel`/`onAcknowledge` is the explicit dismissal path; Escape closing the underlying screen is acceptable v1 behavior. **Trigger:** smoke evidence that Escape-closes-screen-instead-of-dismissing-dialog is jarring, OR multiple consumers want Escape-as-cancel binding. Don't speculate the behavior. Fold-on-evidence inline if surfaced.

- **Modals with slot groups (item-input dialogs).** Phase 14d-1 ships dialogs as decoration primitives — title + body + buttons via M8 composition. They render via `ScreenPanelAdapter` over an existing screen. **Slots inside a modal would require server-side handler + sync protocol** — a separate architectural shape (modal-as-its-own-menu vs modal-as-decoration). Use cases Trevor flagged: "drop item here to confirm" patterns, item-input prompts, multi-slot item-selection dialogs. **Trigger:** ASK TREVOR EXPLICITLY about the desired shape before implementation begins. Architectural questions to resolve at trigger time include: does the modal-with-slots replace the current screen (vanilla confirm-screen pattern) or overlay it (current dialog primitive shape)? How does sync work — through the underlying menu's handler, or through a temporary modal-specific handler? Cross-context applicability — does modal-with-slots work in StandaloneContext too, or MenuContext only? Trevor wants to drive this design conversation when it lands.

- **Click-through prohibition for ALL panels (architectural — surface to advisor).** Smoke during 14d-2 surfaced a foundational principle Trevor named explicitly: *"You should NEVER be able to click through behind a panel to something behind. No click through, no tooltips showing behind, no mouse icon change to clickable, nothing. Everything behind the panel is completely inert."* This is a generalization of the modal mechanism shipped in 14d-1 — modal panels are fully opaque to underlying interaction at every level (clicks, hover, cursor, tooltips). Trevor's principle says ALL panels (modal and non-modal alike) should be fully opaque over their bounds. Current architecture: modal panels have full opacity (per-Panel `cancelsUnhandledClicks` flag drives the input-handler mixins). Non-modal panels: clicks dispatched to elements; if consumed, library `allowMouseClick` hook returns `true` (allow vanilla); slot underneath ALSO sees the click → click-through bug. Hover/cursor/tooltip suppression for non-modal panels ALSO not happening — only modal triggers them. **Architectural shape questions for advisor:**
  - (1) Should `cancelsUnhandledClicks` become the default behavior of every Panel (drop the flag, all panels are opaque)?
  - (2) Or should there be a NEW flag (`opaque(boolean)`?) that's true by default, with explicit opt-out for transparent overlays?
  - (3) Does this affect render order / z-index semantics (panels below opaque panels would be inert — same as modal-behind)?
  - (4) Cross-cutting impact on validator scenarios that may have relied on click-through for testing (V2-V7 use lambda-adapter probes that pass-through clicks; do they break?).
  - (5) Lambda-adapter consumer panels (Finding from 14d-1) currently aren't covered by ANY of the modal mechanisms (click-eat, hover-suppress, etc.) — under this principle they SHOULD be. That's a real expansion of library responsibility for the lambda escape hatch. **Trigger:** advisor verdict in Phase 14d-2 close-out, OR sooner if Trevor wants this addressed before more 14d-N elements ship. Architectural question that affects every primitive shipped from here on; worth resolving before 14d-3+.

- **Lambda-adapter modal-aware extension.** 14d-1's modal mechanism delivers dim coverage and inert-RenderContext suppression for **region-based** `ScreenPanelAdapter` adapters — those registered with `MenuRegion` and dispatched via `ScreenPanelRegistry.menuMatches`. **Lambda-based adapters** (constructed with `ScreenOriginFn` instead of a region) are the escape hatch for custom positioning — they bypass `ScreenPanelRegistry` and render via consumer-side mixins. The two-pass dim and `-1` inert sentinel only apply to registry-managed panels. Lambda-path consumers (validator probes, M3 grafted-slot backdrops, etc.) currently render alongside the modal without dim coverage and without inert-context propagation — their elements still show hover feedback while modal is up. Click eating still works (input-handler mixins are universal); the leak is visual only. **Trigger:** first concrete consumer wanting modal-aware behavior in a lambda adapter (e.g., a future consumer that uses lambda-path positioning AND wants its panel dimmed when a modal is up). Possible mechanisms: (a) consumer opt-in API like `Panel.dimUnderModal(true)` consulted by their custom render code; (b) static helper `ScreenPanelRegistry.applyModalInertness(ctx)` consumers call from their lambda adapter render path. Defer until concrete consumer evidence shapes the API.
