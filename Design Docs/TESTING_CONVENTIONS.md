...
**No chat commands.** Phase 14d-2.7 removed every `/mkverify` command — bare and subcommands. Single command was a transitional rule; the binding rule is single test ENTRY POINT, which the inventory button delivers more directly.
...
- Toggle entries: dialog/scroll/opacity smokes, V4.2 HUD/Inventory decoration, V7 HUD probes, region probes (master + stack-middle).
- Button entries: V0/V1.1/V1.2/V4.1/V5 (full scenario screens), V4.2 Standalone (S2C-driven screen open).

Use the shape that matches the test's nature. A scenario that's a workflow (V0, V1, V4.1, V5) is a button — click opens the screen. A test that's "is a smoke panel visible right now?" is a toggle — flip to control visibility.
...
The mechanical pattern post-14d-2.7:

1. **Write the smoke in validator.** New visibility flag in `MenuKitSmokeState` (or a similar state class for the test). New panel registration via `ScreenPanelAdapter` in `MenuKitSmokeWireup` (or similar wireup class). Same shape as the existing dialog/scroll/opacity smokes.

2. **Add one Hub entry.** Edit `HubHandler.create` — add one entry block at the TOP of the existing entry list (most-recent-at-top). For a visibility-flag test: `addToggle(...)`. For a scenario screen open: `addButton(...)` plus a new case in `MkValidator.dispatchOpenScenario`'s switch.

3. **If the test has automated checks**, add them to `runAllAutomatedThenOpenHub` in `MkValidator` (or as a `runAggregated*` method invoked there). They run automatically when the inventory Test button is clicked.

4. **Done.** No new chat command. No new screen factory boilerplate. No library-side test scaffolding.

The convention's structural test sentence:

> *Can a new smoke test be added by writing one wireup method (in validator), registering one Hub entry (in validator), and optionally adding one auto-check probe — with no new chat command and no library-side test scaffolding?*
...
- **Smoke panel placement** — region-based vs lambda; which `MenuRegion`; auto-size vs pinned; etc.
- **Smoke panel visibility model** — supplier-driven via `showWhen(() -> flag)` is the canonical pattern.
- **Smoke target screens** — `.on(...)` vs `.onAny()` per test.
...
**Scenario:** Phase 14d-3 ships text input. The agent (me) just finished implementation and is about to propose smoke tests.

**Wrong-shaped proposal:**

> "Click the inventory Test button (runs all 17 contracts + 5 validator aggregators). Also click M9 Opacity to verify modals didn't regress. Also click ScrollContainer. Also Modal Dialog. Also Region probes. Also V0..."

**Right-shaped proposal:**

> "Click the inventory Test button — confirms 17/17 contracts + 5/5 aggregators pass. For visual smoke: open the Hub, click the new 'Text Input' entry — verifies focus, IME, validation, submission. Modal/scroll/opacity/regions don't intersect this change so don't need re-smoking."

The right-shaped proposal trusts the inventory Test button as the regression backstop and scopes visual smoke to what's actually new.
...
**Q1 — Single command shape: dissolved.** The original Q1 verdict (`/mkverify` opens Hub + runs contracts) ran for ~half a phase before Trevor surfaced the deeper convention: no chat commands at all. The inventory "Test" button is the entry point; chat commands are gone. Q1 is moot post-14d-2.7.
