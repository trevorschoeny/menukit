...
SlotGroupContext's vanilla-category catalog (M6 §6) is the first concrete invocation of the exception. Per-category cost is one enum constant plus one resolver entry — trivial. Incompleteness cost is high: a consumer targeting a category the library hadn't shipped would find their panel silently failing to appear on that screen class, and would need to migrate against the added category once the library caught up. Both tests are satisfied, and the v1 catalog ships with the 43 vanilla 1.21.11 categories exhaustively covered.
